...
class TrainConfig:
    ...
    save_interval: int = 50000                                       # Interval for saving checkpoints (in steps)
    ...
    cache_dir: Union[str, Path] = Path("/home/hk-project-p0024638/uvrfq/hkfswork/uvrfq-model_cache/ECoT")
...
def train(cfg: TrainConfig) -> None:
    ...
    overwatch.info(f"Loading Base VLM `{cfg.vla.base_vlm}` from ID/Path: {cfg.pretrained_checkpoint}")
    if cfg.pretrained_checkpoint is not None:
        ...
        vlm = load(
            cfg.vla.base_vlm, hf_token=hf_token, load_for_training=True, cache_dir=cache_dir, image_sequence_len=cfg.image_sequence_len
        )
    ...
    vla_dataset, action_tokenizer, collator = get_vla_dataset_and_collator(
        ...
        batch_modifier_fn=reasoning_modifier_fn
    )
