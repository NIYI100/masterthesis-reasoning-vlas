...
def make_dataset_from_rlds(
    ...
    *,
    ...
    reasoning_dataset_path: str = "/home/hk-project-p0024638/uvrfq/hkfswork/ecot-lite_data/libero_reasonings.json",
    ...
    def make_tf_dict(raw_dict):
        ...
        def swap_coordinates_in_string(input_string):
            ...
            updated_string = re.sub(r'VISIBLE OBJECTS:@(\{.*?\})@', replace_objects, input_string)
            updated_string = re.sub(r'GRIPPER POSITION:@(\[.*?\])', replace_gripper, updated_string)
        ...
        # ep_path
        for ep_path in raw_dict.keys():
            # demo_id
            for demo_id in raw_dict[ep_path].keys():

                for step_idx in raw_dict[ep_path][demo_id].keys():
                    keys.append(ep_path + "_" + str(demo_id) + "_" + step_idx)
                    reasoning_dict = raw_dict[ep_path][demo_id][step_idx]

                    raw_string = reasoning_dict_to_str(reasoning_dict)
                    #values.append(raw_string)
                    swapped_string = swap_coordinates_in_string(raw_string)
                    if reasoning_modifier_fn:
                        swapped_string = reasoning_modifier_fn(swapped_string)
                    values.append(swapped_string)
