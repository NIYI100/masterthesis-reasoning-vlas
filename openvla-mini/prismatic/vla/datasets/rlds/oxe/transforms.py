...
def libero_lm_90_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # --- 1. REPO LOGIC: Physics Normalization ---
    # Gripper action is in -1 (open)...1 (close) -> clip to 0...1, flip -> +1 = open, 0 = close
    gripper_action = trajectory["action"][:, -1:]
    gripper_action = invert_gripper_actions(tf.clip_by_value(gripper_action, 0, 1))

    normalized_action = tf.concat(
        [
            trajectory["action"][:, :6],
            gripper_action,
        ],
    ...
    # --- 2. REPO LOGIC: State Formatting ---
    # MiniVLA expects specific keys for proprioception
    # We create a new observation dict to ensure clean keys
    new_observation = {
        "image": trajectory["observation"]["image"],
        "wrist_image": trajectory["observation"]["wrist_image"],
        "EEF_state": trajectory["observation"]["state"][:, :6],      # Required by MiniVLA
        "gripper_state": trajectory["observation"]["state"][:, -2:], # Required by MiniVLA
        # Keep original state just in case, but usually not needed if EEF is present
    ...
    return {
        ...
        "dataset_name": "libero_lm_90", # Explicit naming is good
        
        # --- 4. YOUR FIX: Metadata Nesting ---
        # Keeps your fix to ensure reasoning lookup works later
        "traj_metadata": {
            "episode_metadata": trajectory["traj_metadata"]["episode_metadata"]
        }
    }
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "libero_lm_90": libero_lm_90_dataset_transform
}
