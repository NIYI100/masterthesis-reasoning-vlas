...
class RLDSBatchTransform:
    ...
    batch_modifier_fn: Optional[Callable[str, str]] = None
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reasoning = ""
        if "reasoning" in rlds_batch and rlds_batch["reasoning"] is not None:
            reasoning = rlds_batch["reasoning"].decode()

        if self.batch_modifier_fn:
            reasoning = self.batch_modifier_fn(reasoning)
        ...
        # [Prompt Construction]
        if reasoning:
            full_response_str = f"{reasoning} {CotTag.ACTION.value} {tokenized_action}"
        else:
            full_response_str = f"{CotTag.ACTION.value} {tokenized_action}"
        
        conversation.extend( 
            [
                {"from": "human", "value": f"What action should the robot take to {lang}?"},
                {"from": "gpt", "value": full_response_str},
            ]
        )
        raw_response_tokens = self.base_tokenizer(full_response_str, add_special_tokens=False)["input_ids"]
        num_answer_tokens = len(raw_response_tokens)
