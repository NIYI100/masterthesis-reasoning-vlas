...
class VQActionTokenizer(ActionTokenizer):
    ...
    def __call__(self, action: np.ndarray) -> Union[str, List[str]]:
        ...
        action = torch.from_numpy(action.copy()).to(self.device).reshape((1, self.vq_vae.input_dim_h, self.vq_vae.input_dim_w))
...
ACTION_TOKENIZERS = {
    ...
    "libero_vq_action_tokenizer_10_future": partial(
        VQActionTokenizer, 
        vq_vae_path="vq/pretrain_vq+mx-libero_lm_90+fach-9+ng-7+nemb-256+nlatent-512",
        use_extra=True
    ),
