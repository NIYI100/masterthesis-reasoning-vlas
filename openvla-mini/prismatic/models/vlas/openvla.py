...
class OpenVLA(PrismaticVLM):
    ...
    def predict_action(
        ...
        with torch.autocast("cuda", dtype=autocast_dtype, enabled=self.enable_mixed_precision_training):
            # fmt: off
            generated_ids = self.raw_generate(input_ids, pixel_values)
    ...
    def generate(
        ...
        if injected_reasoning is not None:
            added_tokens = tokenizer(injected_reasoning, add_special_tokens=False, return_tensors="pt").input_ids.to(self.device)
