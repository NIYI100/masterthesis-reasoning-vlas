...
def load(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        with overwatch.local_zero_first():
            if "qwen" in model_id:
                #qwen_repo = "Stanford-ILIAD/minivla-libero90-prismatic"
                qwen_repo = "Stanford-ILIAD/prism-qwen25-extra-dinosiglip-224px-0_5b"
                config_json = hf_hub_download(repo_id=qwen_repo, filename="config.json", cache_dir=cache_dir)
                checkpoint_pt = hf_hub_download(
                    repo_id=qwen_repo, 
                    #filename="checkpoints/step-200000-epoch-45-loss=0.0118.pt",
                    filename="checkpoints/step-020792-epoch-01-loss=0.5268.pt",
                    cache_dir=cache_dir
                )
            else:
    ...
    # === PATCH START: Handle VLA Training Configs ===
    with open(config_json, "r") as f:
        loaded_json = json.load(f)

    # Case A: Standard VLM Config (has "model" key)
    if "model" in loaded_json:
        model_cfg = loaded_json["model"]
    
    # Case B: VLA/MiniVLA Config (has "vla" key)
    elif "vla" in loaded_json:
        overwatch.info("Detected VLA Config format. Generating Base VLM config from ID...")
        
        # 1. Extract the base model ID string
        # e.g. "prism-qwen25-extra-dinosiglip-224px+0_5b"
        base_vlm_id = loaded_json["vla"]["base_vlm"]
        
        # 2. Instantiate the default configuration for this ID
        cfg_obj = ModelConfig.get_choice_class(base_vlm_id)()
        
        # 3. Convert to dictionary so the rest of the script works (model_cfg['key'])
        model_cfg = dataclasses.asdict(cfg_obj)
        model_cfg["finetune_train_strategy"] = "ddp"
    
    else:
        raise ValueError("config.json is missing both 'model' and 'vla' keys!")
    # === PATCH END ===
