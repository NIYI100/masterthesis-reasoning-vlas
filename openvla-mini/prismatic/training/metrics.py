...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            ...
            "whole_accuracy": deque(maxlen=window_size),
            "reasoning_accuracy": deque(maxlen=window_size),
            "action_tag_accuracy": deque(maxlen=window_size),
            # ------------------------------
        }
    ...
    def push(self) -> str:
        ...
        # --- CALCULATE NEW METRICS HERE ---
        whole_accuracy = torch.stack(list(self.state["whole_accuracy"])).mean().item()
        reasoning_accuracy = torch.stack(list(self.state["reasoning_accuracy"])).mean().item()
        action_tag_accuracy = torch.stack(list(self.state["action_tag_accuracy"])).mean().item()
        # ----------------------------------
        ...
        self.log(
            ...
            metrics={
                ...
                # --- LOG NEW METRICS HERE ---
                f"{prefix}/Whole Accuracy": whole_accuracy,
                f"{prefix}/Reasoning Accuracy": reasoning_accuracy,
                f"{prefix}/Action Tag Accuracy": action_tag_accuracy,
                # ----------------------------
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
