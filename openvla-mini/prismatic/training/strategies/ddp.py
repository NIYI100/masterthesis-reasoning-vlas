...
class DDPStrategy(TrainingStrategy):
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        # Gradient checkpointing causes reentrant backward passes (same params used multiple times).
        # DDP would otherwise mark params "ready" twice. _set_static_graph() tells DDP the graph
        # is fixed across iterations, so it can handle this correctly.
        if self.enable_gradient_checkpointing:
            self.vlm._set_static_graph()
